...
//
// 	// ErrEntriesLinkMismatch signals that the link generated from chunking the multihashes returned by provider.MultihashLister does not match the previously generated link. This error is most likely caused by the lister returning inconsistent multihashes for the same key.
...
func mkLinkSystem(remote Remote) ipld.LinkSystem {
	...
	lsys.StorageWriteOpener = func(lctx ipld.LinkContext) (io.Writer, ipld.BlockWriteCommitter, error) {
		...
		return buf, func(lnk datamodel.Link) error {
			// write the data at /<cid>
			return remote.WriteFile(lctx.Ctx, lnk.(cidlink.Link).Cid.String(), buf.Bytes())
		}, nil
	}
